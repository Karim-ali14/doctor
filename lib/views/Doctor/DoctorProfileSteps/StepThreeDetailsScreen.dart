...
class _StepThreeDetailsScreenState extends State<StepThreeDetailsScreen> {
  ...
  selectTime(i, bool isStartTime) async {
    ...
    if (_picked != null) {
      ...
      setState(() {
        if (isStartTime) {
          print((_picked.minute > 10
              ? _picked.minute.toString()
              : "0" + _picked.minute.toString()));
          startTime[i] = (_picked.hour > 10
              ...
              (_picked.minute > 10
                  ...
                  : "0" + _picked.minute.toString());
          print(startTime[i]);
          textEditingControllerStartTime[i].text = (_picked.hour > 10
                  ? _picked.hour.toString()
                  : "0" + _picked.hour.toString()) +
              ":" +
              (_picked.minute > 10
  ...
  addValues() {
    ...
    setState(() {
      ...
      endTime.add("x");
      slotsList.add(List());
      doctorSlotsDetails.data.add(SlotsData(
          dayId: int.parse(widget.id),
          duration: "15",
          doctorId: 1,
                            ...
                            child: Column(
                              children: [
                                ListView.builder(
                                  ...
                                  itemCount: doctorSlotsDetails == null
                                      ? 0
                                      : doctorSlotsDetails.data.length,
  ...
  MyCard(i) {
    return Visibility(
      visible: widget.id == doctorSlotsDetails.data[i].dayId.toString()
          ? true
          : false,
      child: Container(
                ...
                onChanged: (val) {
                  ...
                  setState(() {
                    ...
                    slotsDistribution(startTime[i], endTime[i],
                        int.parse(val.toString().substring(0, 2)), i);
                  });
                },
  ...
  slotsDistribution(String startTime, String endTime, interval, i) {
    ...
    try {
      if (int.parse(startTime.split(":")[1]) >= 60 ||
        ...
        print(d2.difference(d1).inMinutes);
            ...
            setState(() {
              slotsList[i].add(Getslotls(
                  slot: dateTime
                          ...
                          .toString()
                          .substring(11, 16) +
                      "${dateTime.add(Duration(minutes: x * interval)).hour > 12 ? " $PM" : " $AM"}"));
            });
          }
        }
      }
      ...
      for (int i = 0; i < 7; i++) {
        ...
        for (int j = 0; j < doctorSlotsDetails.data.length; j++) {
          ...
          if (i == doctorSlotsDetails.data[j].dayId) {
            z = z + 1;
            x = x +
                "{\"start_time\":\"${startTime[j]}\",\"end_time\":\"${endTime[j]}\",\"duration\":\"${selectedvValue[j].substring(0, 2)}\"}${slotsCount[i] == z ? "" : ","}";
          }
        }
        x = x + "]${i < 6 ? "," : ""}";
      }
  ...
  uploadData(x) async {
    if (widget.base64image == null) {
      ...
      final response =
        ...
        "consultation_fees": widget.myData.consultationFee,
